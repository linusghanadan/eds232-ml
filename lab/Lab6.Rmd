...
[When applied to the testing data, the accuracy of the model was 78%. However, the model made a lot of type I (false-negative) errors, with a specificity of only 43%. This means that when there were in fact eels at the site, the model only correctly predicted that this was the case about 43% of the time. Meanwhile, the sensitivity of the model is high at 90% (i.e., when there were no eels at the site, the model correctly predicted that this was the case 90% of the time).]{style="color:navy;"}
...
eval_predict <- predict(final_fit, eval_data) %>% 
  bind_cols(eval_data) # bind predictions column to testing data
...
[When applied to the evaluation data, the accuracy of the model was 80%, which is slightly better than when the model was applied to the testing data. However, the specificity of the model decreases substantially to only 17% (i.e., when there were eels at the site, the model correctly predicted that this was the case 17% of the time), meaning that the model is committing even more type I errors than it was before. Meanwhile, the sensitivity of the model is extremely high at 98% (i.e., when there were no eels at the site, the model correctly predicted that this was the case 98% of the time).]{style="color:navy;"}
...
[My results are similar to Elith et al in that summer air temperature (seg_sum_t) was the most important predictor variable, with a relative contribution that is above 20%. In addition, for both of us, the second most important predictor variable was the proportion of area with indigenous forest (us_native), though its relative contribution percentage is more than double in my analysis. Meanwhile, one of the most significant differences is the relative contribution of maximum downstream slope (ds_max_slope) in my analysis, which is well ahead of both distance to coast (ds_dist) and the weighted average of proportional cover of bed sediment (loc_sed). For Elith et al, both of these variables were more important than maximum downstream slope. Lastly, another significant difference was the extremely low (basically zero) contribution of days per month with rain greater than 25 mm (us_rain_days) in my analysis, compared to a contribution of 6.5% in Elith et al's analysis. In Elith et al's analysis, train days were more important predictors than average temperature in catchment compared with segment (us_avg_t) and winter air temperature (seg_t_seas), while this was not the case in my analysis.]{style="color:navy;"}

-   What do your variable importance results tell you about the distribution of this eel species?

My variable importance results say that summer air temperature, proportion of area with indigenous forest, maximum downstream slope, and distance to coast are the most important predictors of eel species distribution. In addition, my results say that days per month with rain greater than 25 mm, winter air temperature, 
