...
confusion_matrix <- confusionMatrix(trees_predict_train, reference = trees_baked_train$yr1status)
...
The overall accuracy of the model is 90.4%. This is calculated as the number of correct predictions (16,527 + 6,295) divided by the total number of predictions made (25246).
...
```{r}
# bake recipe with testing data
trees_baked_test <- bake(trees_recipe, new_data = trees_test)
```
...
# Make predictions on your data using the trained model
trees_predict_test <- predict(cv_model4, newdata = trees_baked_train)
```
...
```{r}
# Create the confusion matrix
confusion_matrix <- confusionMatrix(test_predict, reference = trees_baked_test$yr1status)
