...
linus_tracks <- read.csv(here::here("lab", "data", "maxwell_songs.csv"))
...
```{r}
accuracy_knn <- accuracy(test_predict, truth = name, estimate = .pred_class) # get accuracy of testing prediction
sensitivity_knn <- sensitivity(test_predict, truth = name, estimate = .pred_class)
specificity_knn <- specificity(test_predict, truth = name, estimate = .pred_class)
```
...
```{r}
accuracy_tree <- accuracy(test_predict, truth = name, estimate = .pred_class) # get accuracy of testing prediction
sensitivity_tree <- sensitivity(test_predict, truth = name, estimate = .pred_class)
specificity_tree <- specificity(test_predict, truth = name, estimate = .pred_class)
```
...
accuracy_bag <- accuracy(test_predict, truth = name, estimate = .pred_class) # get accuracy of testing prediction
sensitivity_bag <- sensitivity(test_predict, truth = name, estimate = .pred_class)
specificity_bag <- specificity(test_predict, truth = name, estimate = .pred_class)
```
...
```{r}
# Evaluate model performance
accuracy_rf <- accuracy(test_predict, truth = name, estimate = .pred_class) # get accuracy of testing prediction
sensitivity_rf <- sensitivity(test_predict, truth = name, estimate = .pred_class)
specificity_rf <- specificity(test_predict, truth = name, estimate = .pred_class)
...
# Create a data frame
metrics_df <- tibble(
  Method = rep(c("KNN", "Decision Tree", "Bagged Trees", "Random Forest"), each = 3),
  Metric = rep(c("Accuracy", "Sensitivity", "Specificity"), times = 4),
  Value = c(accuracy_knn, sensitivity_knn, specificity_knn, 
            accuracy_dt, sensitivity_dt, specificity_dt,
            accuracy_bagged, sensitivity_bagged, specificity_bagged,
