...
test_predict_knn <- predict(final_fit, tracks_test) %>% # get prediction probabilities for test 
  bind_cols(tracks_test) %>%  # bind to testing column
  mutate(name = as.factor(name))

test_predict_knn <- predict(final_fit, tracks_test, type = "prob") %>% # get prediction probabilities for test 
  bind_cols(test_predict_knn)
```
...
```{r}
accuracy_knn <- accuracy(test_predict_knn, truth = name, estimate = .pred_class) # get accuracy of testing prediction
...
```{r}
# predict testing data
test_predict_tree <- predict(final_tree_fit, tracks_test) %>% # get prediction probabilities for test 
  bind_cols(tracks_test) %>%  # bind to testing column
  mutate(name = as.factor(name))

test_predict_tree <- predict(final_tree_fit, tracks_test, type = "prob") %>% # get prediction probabilities for test 
  bind_cols(test_predict_tree)
...
```{r}
accuracy_tree <- accuracy(test_predict_tree, truth = name, estimate = .pred_class) # get accuracy of testing prediction
...
```{r}
# create workflow for Bagged Tree model
...
```{r}
# predict testing data
test_predict_bag <- predict(fit_bag_tree, tracks_test) %>% # get prediction probabilities for test 
  bind_cols(tracks_test) %>%  # bind to testing column
  mutate(name = as.factor(name))

test_predict_bag <- predict(fit_bag_tree, tracks_test, type = "prob") %>% # get prediction probabilities for test
  bind_cols(test_predict_bag)
```
...
```{r}
# Assuming best_rf_params contains the best hyperparameters from tuning

# Finalize the model with the best parameters
final_rf <- finalize_model(rf_spec, best_rf_params)

# Create a new workflow with the finalized model and the existing recipe
final_rf_workflow <- workflow() %>%
  add_recipe(tracks_recipe) %>%
  add_model(final_rf)

# Now, fit this final workflow to the training data
...
test_predict_rf <- predict(final_rf_fit, tracks_test, type = "prob") %>% # get prediction probabilities for test
...
```{r}
# Evaluate model performance
accuracy_rf <- accuracy(test_predict_rf, truth = name, estimate = .pred_class) # get accuracy of testing prediction
...
# Create a data frame
metrics_df <- tibble(
  Method = rep(c("KNN", "Decision Tree", "Bagged Trees", "Random Forest"), each = 4),
  Metric = rep(c("Accuracy", "Sensitivity", "Specificity", "ROC AUC"), times = 4),
  Value = c(accuracy_knn$.estimate[1], sensitivity_knn$.estimate[1], specificity_knn$.estimate[1], roc_auc_knn$.estimate[1],
            accuracy_tree$.estimate[1], sensitivity_tree$.estimate[1], specificity_tree$.estimate[1], roc_auc_tree$.estimate[1],
            accuracy_bag$.estimate[1], sensitivity_bag$.estimate[1], specificity_bag$.estimate[1], roc_auc_bag$.estimate[1],
            accuracy_rf$.estimate[1], sensitivity_rf$.estimate[1], specificity_rf$.estimate[1], roc_auc_rf$.estimate[1])
)

```

```{r}
ggplot(metrics_df, aes(x = Method, y = Value, fill = Metric)) + 
  geom_bar(stat = "identity", position = "dodge") +
  theme_minimal() +
  labs(y = "Metric Value", x = "Classification Method", title = "Performance Metrics by Classification Method") +
