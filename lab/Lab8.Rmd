...
covtype_df <- readr::read_csv(here::here("lab/data/covtype_sample.csv")) %>% 
  ...
  mutate(across(.cols = matches("wilderness_area|soil_type"),
                .fns = factor)) %>% # convert wilderness and soil type columns to binary factor'
...
[The outcome variable is very skewed right. For this reason, its important that we normalize the cover type variable while pre-processing our models.]{style="color:navy;"}
...
```{r}
# finalize the model
best_rf_params <- show_best(rf_tune_res, "accuracy")
