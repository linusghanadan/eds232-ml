...
[The usefulness of this plot is limited due to several factors. Firstly, it feels unintuitive to think of time of year in terms of days, as it requires a decent amount of mental math to convert from day to month. Another factor making it difficult to usefully interpret the plot is just the fact that there are so many points shown on the graph. Both of these problems can be solved by grouping the pumpkin data based on the month column and plotting the mean price for each month as a bar plot.]{style="color:navy;"}
...
[Variety was most highly correlated with price. This might be because certain varieties of pumpkin are significantly more valuable than others based on the consumer demand and producer supply.]{style="color:navy;"}
...
[One major issue is that by converting the type of package to a numeric variable, we run into problems because of the fact that the magnitude of the package number is entirely meaningless in the context of our interpretation. However, the linear model is taking the magnitude into consideration when generating the line of best fit. For example, if packages 2 and 4 swapped their numbers, the model would better fit the data. Another problem here is that there was no price data on package 3 in the training data.]{style="color:navy;"}
