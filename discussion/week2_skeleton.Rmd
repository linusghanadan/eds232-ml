...
```{r}
lm_model <- linear_reg() %>% 
  set_engine("lm") %>% # Set engine as "lm" from base R
  set_mode("regression")
```
...
```{r}
train_fit <- fit(lm_workflow, train) # Fit the model to the training set
...
```{r}
test_predict <- predict(train_fit, test) %>%  # Predict the testing set
  bind_cols(test) # Bind testing set column to prediction
...
Eventually, we will use some other key packages in tidy models such as `dials` and `tune`
